...
public class HtmlParser {
    ...
    public static NewsArticles.DataType getDataType(String _htmlCode) {
        ...
        String initialTypeLabel = "<datatype>";
        String endTypeLabel = "</datatype>";

        int indexOfInitialTypeLabel = _htmlCode.indexOf(initialTypeLabel);
        int indexOfEndTypeLabel = _htmlCode.indexOf(endTypeLabel);

        if (indexOfInitialTypeLabel < indexOfEndTypeLabel && indexOfEndTypeLabel != - 1 &&
        indexOfInitialTypeLabel != - 1){
            int actualPos =  indexOfInitialTypeLabel + initialTypeLabel.length();
            String wordOfLabel = _htmlCode.substring(actualPos,indexOfEndTypeLabel);
            if (wordOfLabel.equals(String.valueOf(NewsArticles.DataType.Training))) {
    ...
    public static String getLabel (String _htmlCode) {
        ...
        int indexOfInitialLabel = _htmlCode.indexOf(initialLabel);
        int indexOfEndLabel = _htmlCode.indexOf(endLabel);

        if (indexOfEndLabel != -1 && indexOfInitialLabel != -1 && indexOfInitialLabel < indexOfEndLabel){
            return _htmlCode.substring(indexOfInitialLabel + initialLabel.length(),indexOfEndLabel);
        }
